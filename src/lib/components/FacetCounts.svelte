<script lang="ts">
  ...
  function handleFacetClick(facetLabel: string) {
    console.log(`Facet clicked: ${facetLabel}`);
    // Call your Solr query function here, passing the clicked facet label
    // fetchSolrData(facetLabel); // Uncomment and replace with your Solr function

    alert(`clicked with ${facetLabel}`);
  }
</script>
    ...
    {#each getFacetFields(facetCounts.facet_fields) as field}
      ...
      {#each field.facets as facet}
        {#if facet.count}
          <div 
            class="facet-item" 
            on:click={() => handleFacetClick(facet.label)}
